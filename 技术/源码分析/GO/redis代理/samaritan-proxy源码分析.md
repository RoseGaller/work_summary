...
使用 Go 开发的，代理redis集群，实现了热点key的实时统计、大 key 的透明(解)压缩
...
```go
func main() {
   parseFlag() //解析输入参数、创建config、初始化Logger
   initStats()
...
func (u *upstream) createClient(addr string) (*client, error) { //创建client
   ...
   options := []clientOption{
      withKeyCounter(u.hkc.AllocCounter(addr)), //为每个redis实例的连接都创建counter
      withRedirectionCb(u.handleRedirection), //返回了ask、moved
      withClusterDownCb(u.handleClusterDown), //集群宕机
   }
   c, err = newClient(conn, u.cfg, u.logger, options...) //创建client实例
   if err != nil {
      return nil, err
   }
...
func (c *Collector) AllocCounter(name string) *Counter {
   c.rwmu.Lock()
   defer c.rwmu.Unlock()
   counter, ok := c.counters[name] //name为redis服务器地址
   if ok { //已经分配计数器
      return counter
   }
   ...
   counter = NewCounter(c.capacity, cb) //创建计数器
...
func (c *Counter) Incr(key string) {
   ...
   if ok {
      ...
      c.increment(item) //计数值+1
      c.mu.Unlock()
      return
   }

   // record
   if uint8(len(c.items)) >= c.capacity { //最多保留50个key的计数
      c.evict() //移除计数值最小的key
   }
...
func (c *Counter) add(item *itemNode) {
  c.items[item.key] = item //key ->item
   ...
   c.freqHead = fnode //每新增一个key，修改freHead
}
```
...
func (l *listener) serve() {
   ...
   for {
      ...
      l.connsWg.Add(1) //连接数+1
      go func(conn net.Conn) { //每个连接都开启一个协程
         l.handleRawConn(conn) // 处理客户端连接
         l.connsWg.Done() //连接数-1
      }(conn)
   }
}
```
...
```go
func (p *redisProc) handleRequest(req *rawRequest) {
   p.stats.Downstream.RqTotal.Inc() //请求计数+1
   ...
   cmd := string(req.Body().Array[0].Text) //set、zset、get、zget
   hdlr, ok := p.findHandler(cmd)  // 查找对应的handler
   if !ok {//handler不存在
      req.SetResponse(newError(fmt.Sprintf("ERR unsupported command '%s'", cmd)))
      return
   }
...
func handleSimpleCommand(u *upstream, req *rawRequest) { //处理简单命令
   body := req.Body()
   if len(body.Array) < 2 { //请求无效
      req.SetResponse(newError(invalidRequest))
      return
   }

   simpleReq := newSimpleRequest(body) //创建simpleRequest实例
   simpleReq.RegisterHook(func(simpleReq *simpleRequest) {
      req.SetResponse(simpleReq.Response()) //设置响应信息
   })
   key := body.Array[1].Text //key
   u.MakeRequest(key, simpleReq) //
}
```
...
```go
func handleSumResultCommand(u *upstream, req *rawRequest) {
  //"del", "exists", "touch", "unlink"
   sumResultReq, err := newSumResultRequest(req) //创建sumResultRequest
   if err != nil {
      req.SetResponse(newError(err.Error()))
      return
   }

   simpleReqs := sumResultReq.Split() //切分成simpleRequest数组
   for i := 0; i < len(simpleReqs); i++ {
      simpleReq := simpleReqs[i]
      key := simpleReq.Body().Array[1].Text
      u.MakeRequest(key, simpleReq)
...
func (u *upstream) MakeRequestToHost(addr string, req *simpleRequest) {
   ...
   c, err := u.getClient(addr) //获取redis连接
   if err != nil {
      req.SetResponse(newError(err.Error()))
      return
   }
   c.Send(req) //发送请求（将请求放入client的chan中）
}
```
