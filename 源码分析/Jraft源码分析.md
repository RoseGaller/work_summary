...
private void handleElectionTimeout() {
    ...
    this.writeLock.lock();
    try {
      if (this.state != State.STATE_FOLLOWER) {//启动之初默认为STATE_FOLLOWER
        return;
      }
      ...
      if (!allowLaunchElection()) { //判断是否允许运行选举
        return;
      }
...
private boolean allowLaunchElection() {//是否允许选举
    ...
    if (this.serverId.isPriorityNotElected()) { //priority=0,不参与选举
        return false;
    }

    // If this nodes disable priority election, then it can make a election.
    if (this.serverId.isPriorityDisabled()) { //priority<=-1,禁用优先选举功能
        return true;
    }
...
private void preVote() {
    ...
    try {
        ...
        if (this.snapshotExecutor != null && snapshotExecutor.isInstallingSnapshot()) { //j接收到Leader发送的InstallSnapshotRequest请求，正在安装快照
            LOG.warn(
                "Node {} term {} doesn't do preVote when installing snapshot as the configuration may be out of date.",
                getNodeId());
            return;
        }
        if (!this.conf.contains(this.serverId)) { //当前节点的serverId不合法
...
private void electSelf() {
    ...
    try {
        ...
        this.voteTimer.start();   //开启投票超时定时器
        //设置quorum为集群内节点的大多数
...
private void becomeLeader() {
    ...
    this.confCtx.flush(this.conf.getConf(), this.conf.getOldConf()); //追加一条最新的日志
...
public void addRequest(final byte[] reqCtx, final ReadIndexClosure closure) {
    ...
    try {
        ...
        int retryTimes = 0;
        while (true) {
            if (this.readIndexQueue.tryPublishEvent(translator)) { //发布事件
                break;
            } else {
                retryTimes++;
...
public void onEvent(final ReadIndexEvent newEvent, final long sequence, final boolean endOfBatch)
    ...
    if (this.events.size() >= ReadOnlyServiceImpl.this.raftOptions.getApplyBatch() || endOfBatch) {//批量处理
        executeReadIndexEvents(this.events);
        this.events.clear();
    }
}
...
private void readLeader(final ReadIndexRequest request, final ReadIndexResponse.Builder respBuilder,
    ...
    respBuilder.setIndex(lastCommittedIndex); //设置最新提交的索引

    if (request.getPeerId() != null) {//不为空说明请求来自follwer或者learner
...
public synchronized void run(final Status status) {
    ...
    // +1代表包含了leader节点
    if (this.ackSuccess + 1 >= this.quorum) { //超过一半节点响应成功，说明leader仍然是leaeder
        this.respBuilder.setSuccess(true);
        this.closure.setResponse(this.respBuilder.build()); //设置leader节点最新提交的索引
        this.closure.run(Status.OK()); //执行回调ReadIndexResponseClosure
        this.isDone = true;
    } else if (this.ackFailures >= this.failPeersThreshold) { //一半响应失败
...
public void run(final Status status) {
    ...
    try {
        if (readIndexStatus.isApplied(ReadOnlyServiceImpl.this.fsmCaller.getLastAppliedIndex())) {//都已应用到状态机
            ReadOnlyServiceImpl.this.lock.unlock();
            doUnlock = false;
            notifySuccess(readIndexStatus); //用户自定义回调
        } else {
            //leader最新提交的index之前的数据尚未应用到状态机，pendingNotifyStatus对其暂存
...
```java
private void notifyLastAppliedIndexUpdated(final long lastAppliedIndex) { //应用到状态机之后，调用次方法
...
this.scheduledExecutorService.scheduleAtFixedRate(() -> onApplied(this.fsmCaller.getLastAppliedIndex()),
    this.raftOptions.getMaxElectionDelayMs(), this.raftOptions.getMaxElectionDelayMs(), TimeUnit.MILLISECONDS); //ReadOnlyServiceImpl启动时，定时调用
```
      ...
      //查找所有小于等于appliedIndex的status
        ...
        if (statuses != null) {
            ...
            while (it.hasNext()) {
                ...
                it.remove(); //从pendingNotifyStatus移除
            }
