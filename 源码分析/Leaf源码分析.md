...
* [号段模式](#号段模式)
  * [获取Id](#获取id-1)
  * [从数据库加载Segment](#从数据库加载segment)
  * [从SegmentBuffer获取Id](#从segmentbuffer获取id)
...
public synchronized Result get(String key) {//考虑时间回退、毫秒的初始sequence取随机值
    ...
    if (lastTimestamp == timestamp) {
        ...
        if (sequence == 0) {
            //seq 为0的时候表示是下一毫秒时间开始对seq做随机，当前毫秒内生成的序列号已达到上限
            sequence = RANDOM.nextInt(100);
            timestamp = tilNextMillis(lastTimestamp);
        }
    } else {
        //如果是新的ms开始，取随机值（如果从0开始递增并且每个毫秒内只有一个请求的场景下，生成的sequence都相同，在分库分表的情况下，数据都会保存到同一数据库的同一张表中，容易导致数据倾斜）
        sequence = RANDOM.nextInt(100);
    }
...
根据从数据库加载号段的频次，动态调整step

默认情况下，当前号段已经使用10%，则开始下载下一号段
...
public void updateSegmentFromDb(String key, Segment segment) {
    ...
    if (!buffer.isInitOk()) { //尚未初始化
        ...
        if (duration < SEGMENT_DURATION) { //距离上次加载号段时间间隔小于15分钟；高并发下号段消耗的太快，增大步长
...
public Result getIdFromSegmentBuffer(final SegmentBuffer buffer) {//初始化已经完成的情况下
    while (true) {
        ...
        try {
            ...
            //如果下一个segment处不于可切换状态并且当前segment的号段已经使用了10%，则尝试加载新的号段
