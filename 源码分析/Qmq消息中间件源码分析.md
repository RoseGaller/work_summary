...
private void doInvoke(ReceivingMessage message) {
    ...
    if (bigSlaveLag()) { //等待slave同步完成 && 同步请求数量大于50000
        brokerReadOnly(message); //返回BROKER_READ_ONLY
        return;
    }
...
对TransactionSynchronization进行扩展，在事务提交之前，将事务消息插入数据库，在事务提交之后将消息发送到消息服务器，发送消息回复成功将消息删除。
...
//事务提交成功之后，发送消息。避免把发送消息和数据库操作放在一个事务里面，发送消息时间过长时会占用数据库连接，出现长事务，导致锁的释放时间延长
@Override
public void afterCommit() {
